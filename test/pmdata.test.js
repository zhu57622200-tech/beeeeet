...
describe('S15 缓存层 refreshPmIfStale', () => {
  ...
  it('pmCachedList 隐藏已开赛结束窗口后的世界杯单场，保留未来单场', async () => {
    resetAll()
    register({ name: '我', password: 'x', agreedTerms: true })
    vi.useFakeTimers()
    try {
      vi.setSystemTime(new Date('2026-06-13T12:00:00Z'))
      store.pmCache.byId['wcgame-ended'] = {
        id: 'wcgame-ended',
        kind: 'wcgame',
        zhTitle: '韩国 vs 捷克',
        gameDate: '2026-06-11',
        gameStart: Date.parse('2026-06-11T12:00:00Z'),
        groups: { moneyline: [] },
        compliant: true,
      }
      store.pmCache.byId['wcgame-future'] = {
        id: 'wcgame-future',
        kind: 'wcgame',
        zhTitle: '未来 vs 比赛',
        gameDate: '2026-06-20',
        gameStart: Date.parse('2026-06-20T12:00:00Z'),
        groups: { moneyline: [] },
        compliant: true,
      }
      expect(pmCachedList().map((e) => e.id)).toEqual(['wcgame-future'])
    } finally {
      vi.useRealTimers()
    }
  })
