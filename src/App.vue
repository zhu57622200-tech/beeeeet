...
import { store, isLoggedIn, me, unreadCount, unreadFeed, markSeen, canClaimWeeklySupply, expireStaleMatches, finishOnboarding, load } from './store.js'
...
const loggedIn = computed(() => (authBump.value, store.authEpoch, isLoggedIn()))
function onLoggedIn() {
  authBump.value++
  go('home')
...
function notifGo(ev) {
  closeNotif()
  const matchId = typeof ev.ref === 'string' && ev.ref.startsWith('match:') ? ev.ref.slice(6) : ev.ref
  if (matchId && store.matches.some((m) => String(m.id) === String(matchId))) {
...
// 首屏 /me 初始化：弱网失败必须可见、可恢复，401 仍交给 store 的登录态回调。
const FIRST_LOAD_AUTO_RETRIES = 3
const firstLoadError = ref('')
const firstLoadRetrying = ref(false)
const firstLoadRetryScheduled = ref(false)
const firstLoadAttempts = ref(0)
let firstLoadTimer = 0
function clearFirstLoadTimer() {
  clearTimeout(firstLoadTimer)
  firstLoadTimer = 0
  firstLoadRetryScheduled.value = false
}
async function runFirstLoad({ resetAttempts = false } = {}) {
  clearFirstLoadTimer()
  if (resetAttempts) firstLoadAttempts.value = 0
  firstLoadRetrying.value = true
  try {
    await load()
    firstLoadError.value = ''
    firstLoadAttempts.value = 0
  } catch (err) {
    if (err?.code === 'AUTH_REQUIRED' || err?.code === 'AUTH_EXPIRED' || !isLoggedIn()) return
    firstLoadError.value = err?.message || '网络连接不稳定，请稍后重试'
    if (firstLoadAttempts.value < FIRST_LOAD_AUTO_RETRIES) {
      firstLoadAttempts.value++
      const delay = Math.min(1000 * (2 ** (firstLoadAttempts.value - 1)), 4000)
      firstLoadRetryScheduled.value = true
    ...
    firstLoadRetrying.value = false
  }
}
function retryFirstLoad() {
  runFirstLoad({ resetAttempts: true })
}

// 进 App：已登录则扫过期盘 + 首登引导（登录后由 onLoggedIn 触发，这里覆盖"已登录直接进"）。
onMounted(() => {
  if (isLoggedIn()) {
    runFirstLoad() // 带 token 刷新进来：拉 /me + 启动轮询（不调它就永远拿不到服务器数据）
    runExpiry()
    checkOnboarding()
  }
})
onUnmounted(clearFirstLoadTimer)
...
<template>
  ...
  <div v-else class="app-root">
    ...
    <main>
      <div v-if="firstLoadError" class="startup-retry" role="alert">
        <div class="startup-title">网络连接不稳定</div>
        <p>{{ firstLoadError }}</p>
        <p v-if="firstLoadRetryScheduled" class="muted">
          正在重连（{{ firstLoadAttempts }}/{{ FIRST_LOAD_AUTO_RETRIES }}）...
        </p>
        <button
          v-else
          class="btn"
          :disabled="firstLoadRetrying"
          @click="retryFirstLoad"
        >
          {{ firstLoadRetrying ? '重试中...' : '重试' }}
        </button>
      </div>
      <template v-else>
...
.startup-retry {
  ...
  background: var(--bg-card);
  box-shadow: 0 12px 32px rgba(15, 83, 110, 0.08);
  text-align: center;
}
.startup-title {
  margin-bottom: 8px;
