...
import { register, login, recoverAccount, me, EMOJIS, EMOJI_LABELS } from '../store.js'

const emit = defineEmits(['done'])

// 本机已有账号 → 默认进"登录"模式，否则"注册"模式。
const hasAccount = computed(() => !!me()?.name)
const mode = ref(hasAccount.value ? 'login' : 'register') // 'register' | 'login' | 'recover'

const name = ref('')
const password = ref('')
const phone = ref('')
const inviteCode = ref('')
const newPassword = ref('') // 找回时设置的新密码
const emoji = ref(EMOJIS[0]) // 默认选中第一只动物（夜枭）
const err = ref('')
...
async function submit() {
  ...
  try {
    if (mode.value === 'register') {
      await register({ name: name.value, password: password.value, phone: phone.value, inviteCode: inviteCode.value, emoji: emoji.value, agreedTerms: agreed.value })
    } else if (mode.value === 'recover') {
      await recoverAccount({ name: name.value, phone: phone.value, newPassword: newPassword.value })
    } else {
...
<template>
  <div class="login-root">
    <div class="card login-card">
      ...
      <template v-else>
        <label class="field-label">注册时填的手机号</label>
        <input v-model="phone" type="tel" inputmode="numeric" maxlength="11" placeholder="验证你的身份" @keyup.enter="submit" />
        <label class="field-label">设置新密码</label>
        <input v-model="newPassword" type="password" autocomplete="new-password" placeholder="重设一个新密码" @keyup.enter="submit" />
      </template>
      ...
      <div v-if="mode === 'register'" class="register-extra">
        ...
        <label class="field-label">团码</label>
        <input
          v-model="inviteCode"
        ...
        <label class="field-label">挑只动物当头像</label>
        <div class="signup-avatar">
          <div class="avatar-current" aria-label="已选头像">{{ emoji }}</div>
          <div class="avatar-meaning">{{ EMOJI_LABELS[emoji] || '' }}</div>
        </div>
        <div class="signup-emoji-picker">
          <button
            v-for="e in EMOJIS"
            :key="e"
            type="button"
      ...
      <div v-else-if="mode === 'login'" class="hint faint">
        用注册时的昵称+密码登录。
        <button type="button" class="link-btn" @click="gotoRecover">忘记密码？</button>
      </div>
      <div v-else class="hint faint">
        验证「昵称 + 注册手机号」后，直接设置新密码登录。
      ...
      <button class="btn block" :disabled="mode === 'register' && !agreed" @click="submit">
        {{ mode === 'register' ? '创建账号，领 100 万' : mode === 'recover' ? '验证并重设密码' : '进入' }}
      </button>
    </div>
  </div>
</template>
...
.emoji-opt.on {
  border-color: #0f536e;
  background: rgba(93, 220, 213, 0.18);
