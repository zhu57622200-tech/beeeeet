...
function placeBet() {
  betErr.value = ''
  betOk.value = ''
  if (isMatch.value ? !matchPick.value : !myPick.value) {
    betErr.value = isOutright.value ? '请先选一个候选' : isMatch.value ? '请先选一个盘口' : '请先选一个结果'
    return
  }
  try {
    if (isOutright.value) {
...
function send() {
  boardErr.value = ''
  try {
    postPmComment(props.event.id, draft.value)
    draft.value = ''
  } catch (e) {
