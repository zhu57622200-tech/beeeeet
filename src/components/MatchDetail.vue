...
import { store, addComment, settleMatchStore, npcJoin, cancelMatch, npcBetBanker, npcBetPool, myJoinPool, markSideBetFulfilled, nagSideBet, sideBetOverdue, fileAppeal, resolveAppealStore, reportCheat, revealWithDispute, scheduleConsensusVotes, arbiterCandidates, inviteArbiter, scheduleArbiterVerdict } from '../store.js'
...
const m = computed(() => store.matches.find((x) => x.id === props.matchId))
const err = ref('')
...
function joinPool(side) {
  try {
    err.value = ''
    myJoinPool(m.value.id, side, Number(joinStake.value))
  } catch (e) {
...
function startReveal(resultSide) {
  if (!m.value) return
  // 庄家裁定：默认「庄家说了算」直接落账；仅当有假人异议时才转共识投票。
  const r = revealWithDispute(m.value.id, resultSide)
  if (!r || !r.disputed) return // 无异议 → 已直接结算，结算卡自动显示
  // 有异议 → 驱动假人陆续表态（异议者反对、其余同意），复用投票面板。
  // timer 下沉 store 层挂 friendTimers：matchId 按值捕获（防切局后写错局），重置/撤盘随 clearFriendTimers 清（LOOP-2）。
  const meName = store.players.find((p) => p.isMe)?.name || '我'
  scheduleConsensusVotes(
    m.value.id,
    m.value.consensus.voters
      .filter((v) => v !== meName)
      .map((v) => ({ voter: v, vote: r.disputers.includes(v) ? 'reject' : 'agree' }))
  )
}
// 僵局时可邀请的评审候选（没参与本局的人）。
const arbiterCands = computed(() => (m.value ? arbiterCandidates(m.value.id) : []))
// 邀请某人当评审：假人版下被邀评审稍后自动裁定（真人版换成真人收到请求后点裁定）。
function inviteAsArbiter(name) {
  if (!m.value) return
  inviteArbiter(m.value.id, name)
  scheduleArbiterVerdict(m.value.id) // timer 挂 store friendTimers，重置/撤盘统一清（LOOP-2）
}
// 当前提议的实时计票（投票面板用）。
...
const myNet = computed(() => {
  ...
  if (mode.value === 'banker') return m.value.bankerPnl || 0
  if (mode.value === 'pool') {
    const myBets = [...poolA.value, ...poolB.value].filter((b) => !b.npc)
    const stake = myBets.reduce((s, b) => s + b.stake, 0)
    const payout = myBets.reduce((s, b) => s + (b.payout || 0), 0)
    return payout - stake
  }
  // match 约赌：押中净赚(奖池-下注)，押错亏下注额（修S7回归:原漏match分支致"输了显示赢+0"）
  return m.value.result === m.value.ownerSide ? potentialWin.value : -m.value.ownerStake
})

// 本局"我"是否赢（跨三玩法统一口径，给特效/解说用）。
function iWonMatch() {
  if (!m.value) return false
  if (mode.value === 'match') return m.value.result === m.value.ownerSide
  return myNet.value >= 0
}

function callFriend() {
  if (m.value) npcJoin(m.value.id)
}
function withdraw() {
  cancelMatch(m.value.id)
  emit('back')
}
function doComment() {
  if (!commentText.value.trim()) return
  addComment(m.value.id, commentText.value.trim())
  commentText.value = ''
}
...
// 被举报方：约赌的对手(takerName)。无对手则不显示入口。
const reportTarget = computed(() => (mode.value === 'match' ? m.value?.takerName : null))
...
function doReport(kind) {
  ...
  const rep = reportCheat(reportTarget.value, kind, m.value?.id)
  ...
  <div v-if="m">
    ...
    <div class="card">
      ...
      <div class="vs-box">
        <div class="vs-opt" :class="{ mine: mode !== 'banker' && m.ownerSide === 'A' }">
          {{ m.optionA }}
          <span v-if="mode !== 'banker' && m.ownerSide === 'A'" class="badge">我押</span>
        </div>
        <span class="vs-mid">VS</span>
        <div class="vs-opt" :class="{ mine: mode !== 'banker' && m.ownerSide === 'B' }">
          {{ m.optionB }}
          <span v-if="mode !== 'banker' && m.ownerSide === 'B'" class="badge">我押</span>
        </div>
      </div>
      <!-- 约赌：原赔率/下注/奖池 -->
      <div v-if="mode === 'match'" class="grid">
        <div><span class="k">赔率</span><span class="v">{{ m.odds }}</span></div>
        <div><span class="k">我下注</span><span class="v">{{ m.ownerStake.toLocaleString() }}</span></div>
    ...
    <template v-if="mode === 'banker'">
      <div class="card" v-if="m.status !== 'settled'">
        ...
        <button v-if="!m.consensus" class="btn block" style="margin-top:12px" @click="callBettor">📣 喊个朋友来押</button>
    ...
    <template v-else>
    <!-- 等接盘 -->
    <div v-if="m.status === 'open'" class="card">
      ...
      <button class="btn block" @click="callFriend">📣 喊个朋友来接</button>
    ...
    <div v-else-if="m.status === 'matched'" class="card opp-card">
      ...
      <p class="muted small">押中净赚 <b class="up">{{ potentialWin.toLocaleString() }}</b>，押错亏 <b class="down">{{ m.ownerStake.toLocaleString() }}</b>。</p>
    </div>
    ...
    >
      ...
      <div class="settled-line">
        ...
        <template v-if="mode === 'match'"> · 你押的「{{ ownerSideLabel }}」{{ m.result === m.ownerSide ? '✅' : '❌' }}</template>
      ...
      <div class="settled-line muted small" v-if="mode === 'match' && m.takerName">
        对手 {{ m.takerEmoji }} {{ m.takerName }} {{ m.result === m.ownerSide ? '吃瘪了' : '赢麻了' }}
      </div>
      <!-- 坐庄：逐笔结果 -->
