...
import {
  ...
  changePassword, deleteAccount, EMOJIS, isOnline,
} from '../store.js'
...
// 联机版 store.matches 是全站大厅列表，"我的对赌"要用独立的 myMatches（owner/taker/下注）；
// 单机版没有 myMatches 数据源，store.matches 本就是"我的"，按模式取。
const myMatchList = computed(() => (isOnline() ? store.myMatches : store.matches))
...
// 退出登录：仅清本机登录态，账号与数据在服务器保留，回到登录页可再登入。
function doLogout() {
  if (confirm('退出登录后回到登录页，账号和数据都保留，下次用昵称+密码登录即可。确定退出？')) resetAll()
}

// ── 修改密码 ──
const showChpwd = ref(false)
const oldPwd = ref('')
const newPwd = ref('')
const chpwdErr = ref('')
const chpwdBusy = ref(false)
function openChpwd() {
  oldPwd.value = ''
  newPwd.value = ''
  chpwdErr.value = ''
  showChpwd.value = true
}
function doChangePassword() {
  chpwdErr.value = ''
  if (!oldPwd.value || !newPwd.value) { chpwdErr.value = '请填写原密码和新密码'; return }
  if (newPwd.value.length < 6) { chpwdErr.value = '新密码至少 6 位'; return }
  chpwdBusy.value = true
  Promise.resolve(changePassword({ oldPassword: oldPwd.value, newPassword: newPwd.value }))
    .then(() => { showChpwd.value = false; alert('密码已修改') })
    .catch((e) => { chpwdErr.value = e?.message || '修改失败' })
    .finally(() => { chpwdBusy.value = false })
}

// ── 注销账号 ──（清空该账号全部信息、释放手机号，完成后退出，可用原号重注册）
const showDelAccount = ref(false)
const delPwd = ref('')
const delErr = ref('')
...
function openDelAccount() {
  delPwd.value = ''
  delErr.value = ''
  showDelAccount.value = true
}
function doDeleteAccount() {
  delErr.value = ''
  if (!delPwd.value) { delErr.value = '请输入密码确认'; return }
  delBusy.value = true
  Promise.resolve(deleteAccount({ password: delPwd.value }))
    .then(() => { showDelAccount.value = false }) // 注销=退出，App 的 loggedIn 会自动回登录页
    .catch((e) => { delErr.value = e?.message || '注销失败' })
...
<template>
  <div>
    ...
    <div class="card">
      <div class="sec-title">我的对赌（{{ myMatchList.length }}）</div>
      <div v-if="myMatchList.length === 0" class="muted small">暂无</div>
      <div
        v-for="mt in myMatchList"
        :key="mt.id"
        class="line"
    ...
    <div class="settings">
      ...
      <button class="btn ghost block" @click="openChpwd">🔑 修改密码</button>
      <button class="btn ghost block" @click="doLogout">↩️ 退出登录</button>
      <button class="btn ghost block danger" @click="openDelAccount">⚠️ 注销账号</button>
    </div>

    <!-- 修改密码弹窗 -->
    <div v-if="showChpwd" class="acct-mask" @click.self="showChpwd = false">
      <div class="acct-sheet">
        <div class="acct-head">修改密码</div>
        <input v-model="oldPwd" type="password" autocomplete="current-password" placeholder="原密码" />
        <input v-model="newPwd" type="password" autocomplete="new-password" placeholder="新密码（至少 6 位）" />
        <div v-if="chpwdErr" class="acct-err">{{ chpwdErr }}</div>
        <div class="acct-actions">
          <button class="btn ghost" @click="showChpwd = false">取消</button>
          <button class="btn" :disabled="chpwdBusy" @click="doChangePassword">{{ chpwdBusy ? '提交中…' : '确认修改' }}</button>
        </div>
      </div>
    </div>

    <!-- 注销账号弹窗 -->
    <div v-if="showDelAccount" class="acct-mask" @click.self="showDelAccount = false">
      <div class="acct-sheet">
        <div class="acct-head danger-text">⚠️ 注销账号</div>
        <p class="acct-warn">
          注销会<b>永久清空你的全部账号信息</b>（积分、战绩、好友、私信等），且<b>不可恢复</b>。
          未结的赌局会作废、冻结积分退还给对手。完成后自动退出，你可以用<b>原手机号重新注册</b>。
        </p>
        <input v-model="delPwd" type="password" autocomplete="current-password" placeholder="输入密码确认注销" />
        <div v-if="delErr" class="acct-err">{{ delErr }}</div>
        <div class="acct-actions">
          <button class="btn ghost" @click="showDelAccount = false">我再想想</button>
          <button class="btn danger" :disabled="delBusy" @click="doDeleteAccount">{{ delBusy ? '注销中…' : '确认注销' }}</button>
        </div>
      </div>
    </div>
  </div>
...
.acct-head { font-size: 17px; font-weight: 700; }
.acct-head.danger-text { color: var(--red, #e2483d); }
.acct-warn { font-size: 13px; line-height: 1.6; color: var(--ink-2, #556); margin: 0; }
.acct-sheet input {
  width: 100%;
  height: 44px;
  border-radius: 10px;
  border: 1px solid rgba(15, 83, 110, 0.18);
  padding: 0 12px;
  font-size: 15px;
  box-sizing: border-box;
}
.acct-err { color: var(--red, #e2483d); font-size: 13px; }
.acct-actions { display: flex; gap: 10px; }
.acct-actions .btn { flex: 1; }
.btn.danger { background: var(--red, #e2483d); color: #fff; border-color: transparent; }
</style>